...
const Footer = () => {
    ...
    const contactLinks: LinkI[] = [
        {
            label: "Head Office: Kampala, Uganda ",
            link: "",
        },
        {
            label: "Pilot Hub: West Nile, Arua District",
            link: "",
        },
        {
            ...
            <Container >
                <div className="flex gap-10">
                    ...
                    <div className="flex-1">
                        ...
                        {links?.map((l, i) => <Link key={i} className='text-white mt-4 block' href={l.link}>{l.label}</Link>)}
                    </div>
                    <div className="flex-1">
                        <div className="text-xl font-bold">Get Involved</div>
                        {involvmentLinks?.map((l, i) => <Link key={i} className='text-white mt-4 block' href={l.link}>{l.label}</Link>)}
                    </div>
                    <div className="flex-1">
                        <div className="text-xl font-bold">Contact Us</div>
                        {contactLinks?.map((l, i) => <Link key={i} className='text-white mt-4 block' href={l.link}>{l.label}</Link>)}
                    </div>
                </div>
            </Container>
        </div>
